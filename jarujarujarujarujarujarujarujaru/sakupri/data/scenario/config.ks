...
;	画面右上の「Back」ボタン
	...
	[image layer=1 left=50 top=30 storage="title/button_config.png" folder="image" visible="true"]
...
;コンフィグボリュームを変更。今流れている曲には反映しない。
[bgmopt volume="&tf.current_bgm_vol" effect="false"]

;ボリュームを即時変更。
[iscript]
TG.kag.tmp.map_bgm[0].volume(tf.current_bgm_vol * 0.3 * 0.01);
[endscript]

[call target="*show_text_sample_bgm"]
...
[call target="*show_text_sample_se"]
...
;テキストサンプルをランダムに表示。BGMをかえたとき
*show_text_sample_bgm

	[iscript]

	var words = [
		"私が音量を調節してあげたわよ。感謝しなさい。",
		"まったく、いつまで設定画面にいるのよ。早く続きをやりましょ。",
		"ボリュームを変更しました！",
		"これくらいのボリューム感でどうっすか？",
		"音が大きすぎるとびっくりしちゃうよね。",
		"いい曲だよね。癒される～。",
	...
	tf.index = Math.floor(Math.random() * words.length);
	tf.text_sample = words[tf.index];

	[endscript]

;	テキスト表示速度のサンプルに使用するメッセージレイヤの設定
	[position layer="message1" left="90" top="520" width="1100" height="100" margint="2" marginl="30" page="fore" visible="true" opacity="0"]
...
;テキストサンプルをランダムに表示。SEをかえたとき
*show_text_sample_se

	[iscript]

	var words = [
		"これくらいのヴォリュームで話せばいいのかしら？",
		"私の声、何回でも聞いていられるわね。",
		"これくらいの声の大きさでどうでしょうか？",
		"（私の声、ちゃんと届いてるのかな……）",
		"これくらいのボリュームで大丈夫かな？うん、いい感じだね。",
		"大きい声でしゃべるの、あんまり得意じゃないんだよね……",
	]
	...
	[elsif exp="tf.index < 4"]
	[playse storage="mio/mio2_03.wav"]
	[elsif exp="tf.index < 6"]
	[playse storage="aoi/aoi1_03.wav"]
	[endif]
...
*show_text_sample
	...
	var words = [
		"これくらいの速さでしゃべるから、覚悟しなさい。",
		"話すスピード、大丈夫かな？",
		"明石もこれくらいの速さでしゃべります！",
		"この速さで読みづらくない？大丈夫？",
		"こんなスピードでしゃべらせる気？まあいいけど。",
		"しゃべりすぎて疲れてきました……ふえぇ。",
	]

	tf.text_sample = words[Math.floor(Math.random() * words.length)];
